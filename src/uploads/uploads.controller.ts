...
import { kindForMime } from './media-library/collections';
...
type AllowedUploadKind = 'image' | 'video' | 'audio' | 'media';

export function uploadFileFilter(allowed: AllowedUploadKind) {
...
export class UploadsController {
  ...
  @Post('profile-photo')
  @UseInterceptors(FileInterceptor('file', { storage: memoryStorage(), fileFilter: uploadFileFilter('image'), limits: { fileSize: maxImageBytes, files: 1 } }))
  ...
  @Post('actsnap-media')
  @UseInterceptors(FileInterceptor('file', { storage: memoryStorage(), fileFilter: uploadFileFilter('media'), limits: { fileSize: maxVideoBytes, files: 1 } }))
  ...
  @Post('videos')
  @UseInterceptors(FileInterceptor('file', { storage: memoryStorage(), fileFilter: uploadFileFilter('video'), limits: { fileSize: maxVideoBytes, files: 1 } }))
  video(@UploadedFile() file: Express.Multer.File) {
    return this.media.upload(file, 'generic-video');
  }

  @Post('audio')
  @UseInterceptors(FileInterceptor('file', { storage: memoryStorage(), fileFilter: uploadFileFilter('audio'), limits: { fileSize: maxAudioBytes, files: 1 } }))
  ...
  @Post('media/batch')
  @UseInterceptors(FilesInterceptor('files', maxBatchFiles, { storage: memoryStorage(), fileFilter: uploadFileFilter('media'), limits: { fileSize: maxBatchFileBytes, files: maxBatchFiles } }))
