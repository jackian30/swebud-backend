...
export class KlipyService {
  ...
  async search(q = '', type: KlipyType = 'gifs', limit = 24) {
    ...
    if (!key) return { results: [], next: null, configured: false, source: 'klipy' };
    ...
    if (!response?.ok) return { results: [], next: null, configured: true, source: 'klipy' };
  ...
  private toMedia(item: any, kind: KlipyType) {
    ...
    const width = original?.gif?.width ?? original?.webp?.width ?? item?.width ?? item?.w ?? item?.dimensions?.width ?? item?.media?.gif?.dims?.[0] ?? null;
    const height = original?.gif?.height ?? original?.webp?.height ?? item?.height ?? item?.h ?? item?.dimensions?.height ?? item?.media?.gif?.dims?.[1] ?? null;
    return {
